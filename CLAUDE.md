...
## Playable lessons so far (66 nodes — ALL 14 domains COMPLETE + module exams; math deepened into mini-courses)
...
## Next (resume here)
STATE as of last session: 66 playable nodes; ALL 14 domains COMPLETE + a module exam each; the 3 robotics-critical MATH nodes (linalg/prob/calculus) deepened into 3-lesson mini-course chains. Build green, content validator clean (incl. one-exam-per-domain invariant), math region + AI/algorithms corridors screenshot-verified, math-linalg-2 played end-to-end incl. its code screen. Committed on `main`.
...
1. Deepen the remaining robotics-critical nodes still at single-lesson depth: **robo-control** (PID tuning / reading the response / intro to state-space) and **programming** (already 3 topic-nodes variables/functions/data, but each could gain worked-problem depth + bigger extraPractice). Follow the math-chain pattern: intro→-2→-3 columns, repoint any long downstream corridor to the chain bottom, screenshot-verify no edge crosses a node. robo-control sits in the denser robotics band (x1520,y880) → check the robo-exam gating + phys-forces/robo-sensing edges when placing depth nodes.
