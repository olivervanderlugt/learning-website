...
1. **Cowork research is landing at `docs/curriculum-gap-analysis.md`** (bachelor-curriculum gap analysis for the Depth stage: canonical syllabi per domain, gap rankings vs current coverage, live-verified problem sources & resource shortlists, suggested build order — it was still being written when this handoff was committed, so check it's finished before relying on it; it's deliberately left uncommitted until done). The next session's job: read it, then fold it into the "Stage 1 — structure & backlog" section above — re-rank the backlog, add missing topics as concrete chain entries, adopt its problem sources into `extraPractice` pools and `resourcesByNode`, and commit the report alongside. Don't start implementing curriculum content before reading it.
